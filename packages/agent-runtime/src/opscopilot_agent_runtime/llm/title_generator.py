...
class LlmTitleGenerator(LlmNodeBase):
    ...
    def generate(self, prompt: str, session_id: str, run_id: str) -> str:
        _logger.debug("LlmTitleGenerator.generate called model=%s [session=%s run=%s]", self._model_id, session_id, run_id)
        ...
        with self._tracer.start_as_current_span("title-gen") as span:
            ...
            _logger.debug("LlmTitleGenerator calling LLM [session=%s run=%s]", session_id, run_id)
            response = self._call(request=request, agent_node="title_gen", recorder=None)
            if response.error or response.output is None:
                _logger.debug("LlmTitleGenerator LLM returned error or empty output error=%r [session=%s run=%s]", response.error, session_id, run_id)
                span.set_attribute("langfuse.trace.output", "")
                return ""
            title = (response.output.text or "").strip().strip('"\'')
            _logger.debug("LlmTitleGenerator LLM returned title=%r [session=%s run=%s]", title, session_id, run_id)
